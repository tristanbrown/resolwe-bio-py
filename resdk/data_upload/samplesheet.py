...
class FileImporter(object):
    ...
    def __init__(self, annotation_path):
        ...
        self.sample_list, self.invalid_samples = self._create_all_samples(entry_list)
    ...
    def _create_all_samples(self, entries):
        ...
        for entry in entries:
            valid_samples, invalid_samples = (
                self._create_sample(entry, valid_samples, invalid_samples)
            )
        return valid_samples, invalid_samples

    def _create_sample(self, entry, valid_samples, invalid_samples):
        """Create a sample from a samplesheet entry, if unique."""
        name = entry['SAMPLE_NAME']
        if name in invalid_samples or name in valid_samples:
            logger.warning(
                "The sample name '%s' is duplicated. Please use unique "
                "sample names.",
                name
            )
            invalid_samples.add(name)
...
class Sample(object):
    ...
    def _get_community_tag(self, experiment):
        ...
        experiment = self.seq_type.lower()
